...
from src import networks_embed as crl_networks
...
class WordEmbeddingHandler:
    def __init__(self, embedding_path: str):
        import gensim
        self.model = gensim.models.KeyedVectors.load(embedding_path)
    ...
    def compute_composite_embedding(self, action: str, object_type: str):
        if action not in self.model or object_type not in self.model:
            raise ValueError(f"Action '{action}' or object '{object_type}' not found in embeddings.")
        return self.model[action] + self.model[object_type]

    def compute_similarity(self, vector1: jnp.ndarray, vector2: jnp.ndarray):
        #return jnp.dot(vector1, vector2) / (jnp.linalg.norm(vector1) * jnp.linalg.norm(vector2) + 1e-6)
        return 0.5
        
# Load word embeddings
embedding_path = "/scratch/network/sa3937/iw/JaxGCRL/word2vec.model"
embedding_handler = WordEmbeddingHandler(embedding_path)
embedding_dim = embedding_handler.vector_size

training_embedding = embedding_handler.get_embedding("binpick")
testing_embedding = embedding_handler.get_embedding("push")
similarity = embedding_handler.compute_similarity(training_embedding, testing_embedding)
bias_embedding = {"similarity": similarity}
...
def bias_actions_with_embeddings(embedding_handler, action_probs, current_action_object, target_action_object):
    similarity = embedding_handler.compute_similarity(current_action_object, target_action_object)
    action_probs = action_probs * similarity
    action_probs = action_probs / jnp.sum(action_probs)  # Normalize probabilities
    return action_probs
...
def actor_step(
    ...
    policy: Policy,
    key: PRNGKey,
    training_embedding: jnp.ndarray,
    extra_fields: Sequence[str] = (),
) -> Tuple[State, Transition]:
    """Collect data with biased actions."""
    # Step 1: Get actions and their logits from the policy
    actions, policy_extras = policy(env_state.obs, key)  # Shape: [batch_size, num_actions]

    # Step 2: Compute action probabilities (softmax) for each environment in the batch
    action_probs = jax.nn.softmax(actions)  # Shape: [batch_size, num_actions]

    # Step 3: Bias action probabilities based on training environment embedding
    obs_embedding = embedding_handler.get_embedding("push")
    similarity = embedding_handler.compute_similarity(training_embedding, obs_embedding)

    biased_action_probs = action_probs * similarity
    biased_action_probs /= biased_action_probs.sum(axis=-1, keepdims=True)  # Normalize

    # Step 4: Sample actions using the biased probabilities
    action_indices = jnp.arange(actions.shape[-1])  # Range: [0, num_actions]
    keys = jax.random.split(key, biased_action_probs.shape[0])  # One key per batch

    sampled_actions = jax.vmap(
        lambda probs, k: jax.random.choice(k, action_indices, p=probs)
    )(biased_action_probs, keys)  # Shape: [batch_size]

    # Step 5: Reshape sampled_actions if necessary
    if len(sampled_actions.shape) == 1:  # Ensure it matches environment expectations
        sampled_actions = sampled_actions[:, None]

    # Step 6: Perform the environment step
    nstate = env.step(env_state, sampled_actions)  # Pass batched actions to the environment

    # Step 7: Collect and return transition data
    state_extras = {x: nstate.info[x] for x in extra_fields}
    return nstate, Transition(
        observation=env_state.obs,  # Keep the original observation
        action=sampled_actions,    # Batched actions
...
def train(
    ...
    biased_policy = functools.partial(
        make_policy,
        deterministic=deterministic_eval,
        bias_embedding=bias_embedding,  # Pass the pre-computed bias embedding
    )
    
    evaluator = CrlEvaluator(
        eval_env,
        biased_policy,  # Use the biased policy
