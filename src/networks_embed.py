...
def make_inference_fn(crl_networks: CRLNetworks):
    ...
    def make_policy(params: types.PolicyParams, deterministic: bool = False, bias_embedding: Optional[dict] = None) -> types.Policy:
        def policy(obs: types.Observation, key_sample: PRNGKey) -> Tuple[types.Action, types.Extra]:
            ...
            if bias_embedding is not None:
                action_probs = jax.nn.softmax(logits)
                similarity = bias_embedding["similarity"]
                biased_action_probs = action_probs * similarity
                biased_action_probs = biased_action_probs / jnp.sum(biased_action_probs)  # Normalize
                logits = jnp.log(biased_action_probs)  # Reconstruct logits for softmax
